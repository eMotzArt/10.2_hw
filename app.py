from flask import Flask, abort
from classes import RepositoryCandidates, Preformater, AppTemplateRenderer as Renderer

app = Flask(__name__)

CANDIDATES_FILE: str = 'candidates.json'

@app.route('/')
def page_index():
    candidates = RepositoryCandidates(CANDIDATES_FILE).get_all()
    if candidates:
        return Renderer.render_index(candidates)
    else:
        abort(404)


@app.route('/candidates/<int:candidate_number>')
def page_per_num(candidate_number):
    candidate = RepositoryCandidates(CANDIDATES_FILE).get_by_id(candidate_number)
    if candidate:
        return Renderer.render_by_id(candidate)
    else:
        abort(404)

@app.route('/skills/<skill>')
def page_per_skills(skill: str):
    candidates = RepositoryCandidates(CANDIDATES_FILE).get_by_skill(skill)
    if candidates:
        return Renderer.render_by_skill(candidates)
    else:
        abort(404)

if __name__=='__name__':
    app.run(host='127.0.0.2', port=80)
